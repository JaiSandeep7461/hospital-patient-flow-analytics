...
# Paths
silver_path = "abfss://silver@hospitalstorage01022026.dfs.core.windows.net/patient_flow"
gold_dim_patient = (
    "abfss://gold@hospitalstorage01022026.dfs.core.windows.net/dim_patient"
)
gold_dim_department = (
    "abfss://gold@hospitalstorage01022026.dfs.core.windows.net/dim_department"
)
gold_fact = (
    "abfss://gold@hospitalstorage01022026.dfs.core.windows.net/fact_patient_flow"
)
...
incoming_patient = silver_df.select("patient_id", "gender", "age").withColumn(
...
if not DeltaTable.isDeltaTable(spark, gold_dim_patient):
    ...
    incoming_patient.withColumn(
        "surrogate_key", F.monotonically_increasing_id()
    ).withColumn("effective_to", lit(None).cast("timestamp")).withColumn(
        "is_current", lit(True)
    ).write.format(
        "delta"
    ).mode(
...
if changed_keys:
    ...
    target_patient.update(
        condition=expr(
            "is_current = true AND surrogate_key IN ({})".format(
                ",".join([str(k) for k in changed_keys])
            )
        ),
        set={"is_current": expr("false"), "effective_to": expr("current_timestamp()")},
    )
...
incoming_dept = incoming_dept.dropDuplicates(["department", "hospital_id"]).withColumn(
    "surrogate_key", monotonically_increasing_id()
)

# initialize table if missing

incoming_dept.select("surrogate_key", "department", "hospital_id").write.format(
    "delta"
).mode("overwrite").save(gold_dim_department)
...
dim_dept_df = (
    ...
    .select(
        col("surrogate_key").alias("surrogate_key_dept"), "department", "hospital_id"
    )
)
...
fact_enriched = fact_base.join(dim_patient_df, on="patient_id", how="left").join(
    dim_dept_df, on=["department", "hospital_id"], how="left"
)

# Compute metrics
fact_enriched = (
    fact_enriched.withColumn(
        "length_of_stay_hours",
        (
            F.unix_timestamp(col("discharge_time"))
            - F.unix_timestamp(col("admission_time"))
        )
        / 3600.0,
    )
    .withColumn(
        "is_currently_admitted",
        F.when(col("discharge_time") > current_timestamp(), lit(True)).otherwise(
            lit(False)
        ),
    )
...
print(
    "Department dim count:",
    spark.read.format("delta").load(gold_dim_department).count(),
)
